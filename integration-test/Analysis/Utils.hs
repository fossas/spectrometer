...
import Graphing (vertexList, directList, countEdges)
...
data Assertion = Assertion 
  ...
  DependencyResultsTabulated deriving (Show, Eq, Ord)
...
testSuiteOf AnalysisIntegrationCase{..} = do
  ...
  where
    ...
    downloadArtifact target = sendIO $ do
      ...
      case resolvedUrl of
        Nothing -> fail ("could not resolved, artifact's download url: " <> show artifactUrl)
        Just (url, _) -> do
