...
execS3Upload :: (Has Trace sig m, MonadIO m) => Path Abs Dir -> Text -> IPROpts -> m ()
execS3Upload basedir scanId IPROpts{..} = do
  ...
  liftIO $ runTrace $ uploadFileToS3 cfg s3cfg mgr filePath scanId

uploadFileToS3 :: (Has Trace sig m, MonadIO m) => Aws.Configuration -> S3.S3Configuration Aws.NormalQuery -> Manager -> String -> Text -> m ()
