...
module App.Fossa.VPS.Ninja.Type
  ( Str,
    FileStr,
    Expr (..),
    Env,
...
import App.Fossa.VPS.Ninja.Env (Env, addEnv, askEnv, newEnv)
...
askExpr e = f
  where
    f (Exprs xs) = BS.concat <$> mapM f xs
    f (Lit x) = pure x
    f (Var x) = askVar e x
