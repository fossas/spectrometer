...
spec = do
  ...
  describe "applyFiltersNew" $ do
    it "should fail when all filters fail" $ do
      let testConfigOne = Just $ ConfigFile 1 Nothing Nothing Nothing Nothing configTargets1 Nothing 
    ...
    it "should only accept specif build targets when they are mentioned" $ do
      let testConfigOne = Just $ ConfigFile 1 Nothing Nothing Nothing Nothing configTargets1 Nothing 
          configTargets1 = Just $ ConfigTargets [ConfigTarget (T.pack "mvn") (Just "baz")] []
      applyFiltersNew testConfigOne "mvn" $(mkRelDir "bar") (S.fromList [BuildTarget "baz", BuildTarget "quux"]) `shouldBe` Just (S.fromList [BuildTarget "baz", BuildTarget "quux"])
